...
class ident(atom, recordtype("ident", "name lineno column lexpos defs props init", default=None)):
    def __str__(self):
        return self.name
...
class let(stmt, recordtype("let", "ret args lineno lexpos nargout", default=None)):
    """Assignment statement, except [x,y]=foo(x,y,z), which is handled by call_stmt."""
    def __str__(self):
        return "%s=%s" % (str(self.ret), str(self.args))

class func_stmt(stmt, recordtype("func_stmt", "ident ret args stmt_list use_nargin", default=None)):
    pass
...
class builtins(funcall):
    """
    Builtin functions are represented as subclasses of class builtins. Application of a function to
    specific arguments is represented as its instance. For example, builtin function foo is represented
    as class foo(builtins), and foo(x) is represented as foo(x).
    """
    def __init__(self, *args, **kwargs):
        """
        If a built-in function _foo takes three arguments a, b, and c, we can just say _foo(a,b,c) and let
        the constructor take care of proper structuring of the node (like expr_list around the arguments, etc.
        """
...
class expr(node, recordtype("expr", "op args")):
    def __str__(self):
        ...
        if len(self.args) == 2:
            return "%s%s%s" % (self.args[0]._backend(), self.op, self.args[1]._backend())
